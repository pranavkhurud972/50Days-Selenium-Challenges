...
public class _8_DragAndDrop {
	...
	public void SortListOrderTest() {
		...
		String[] topList = { "Jeff Bezos", "Bill Gates", "Warren Buffett", "Bernard Arnault", "Carlos Slim Helu",
		...
		for (String name : topList) {
			WebElement draggableItem = wait.until(
					ExpectedConditions.visibilityOfElementLocated(By.xpath("//p[contains(text(), '" + name + "')]")));
			WebElement targetPosition = wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("//span[contains(text(), '" + counter + "')]")));
